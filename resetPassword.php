...
if (isset($_POST['resetPasswordUserEmail'])) {
  $resetPasswordUserEmail = htmlentities($_POST['resetPasswordUserEmail']);
  ...
  if (!empty($resetPasswordUserEmail) && !empty($resetPasswordPassword1) && !empty($resetPasswordPassword2)) {
    if ($resetPasswordUserEmail == '') {
      $resetPasswordMessage = '<div class="alert alert-danger">Please enter your email.</div>';
    }
    elseif ($resetPasswordPassword1 == '' || $resetPasswordPassword2 == '') {
      $resetPasswordMessage = '<div class="alert alert-danger">Please enter both passwords.</div>';
    }
    else {
      $emailCheckingSql = 'SELECT * FROM user WHERE userEmail = :userEmail';
      $emailCheckingStatement = $conn->prepare($emailCheckingSql);
      $emailCheckingStatement->execute(['userEmail' => $resetPasswordUserEmail]);

      if ($emailCheckingStatement->rowCount() < 1) {
        $resetPasswordMessage = '<div class="alert alert-danger">Email does not exist.</div>';
      } else {
        ...
        if ($resetPasswordPassword1 !== $resetPasswordPassword2) {
          ...
          $updatePasswordSql = 'UPDATE user SET password = :password WHERE userEmail = :userEmail';
          $updatePasswordStatement = $conn->prepare($updatePasswordSql);
          $updatePasswordStatement->execute(['password' => $resetPasswordPassword1, 'userEmail' => $resetPasswordUserEmail]);
      ...
      <div class="col-12">
        ...
        <nav class="navbar navbar-expand-lg blur border-radius-lg top-0 z-index-3 shadow position-absolute mt-4 py-2 start-0 end-0 mx-4">
          <div class="container-fluid">
            <a class="navbar-brand font-weight-bolder ms-lg-0 ms-3" href="resetPassword.php">
                ...
                <div class="card-body shadow-lg rounded">
                  ...
                  <form method="POST"  class=" form-control-lg">
                    <div class="form-group">
                      <label for="resetPasswordUserEmail">Email</label>
                      <input type="text" class="form-control" id="resetPasswordUserEmail" name="resetPasswordUserEmail">
                    </div>
                    <div class="form-group">
